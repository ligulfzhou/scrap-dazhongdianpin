...
yuhangqu = pymongo.MongoClient('127.0.0.1:27017').yuhangqu
...
def insert_into_mongodb(dbname, name, categoty, mobile):
    ...
    yuhangqu.store.insert({'name': name, 'mobile': mobile, 'category': categoty})
...
urls = url_with_page('http://www.dianping.com/search/category/3/45/r8845', 17)
...
def scrapy():
    for url in urls:
        ...
        for li in lis:
            ...
            insert_into_mongodb('yuhangqu', a_s[1]['title'], li.find('span', attrs={'class': 'tag'}).text, mobiles)
        # time.sleep(0.2)
